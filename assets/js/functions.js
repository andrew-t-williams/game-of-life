...
canvas.addEventListener("mousedown", function (event) {
    ...
    for (var pattern_x = 0; pattern_x < pattern.length; pattern_x++) {
        for (var pattern_y = 0; pattern_y < pattern[0].length; pattern_y++) {
            var adjusted_x = x + pattern_x - Math.floor(pattern.length / 2)
            var adjusted_y = y + pattern_y - Math.floor(pattern[0].length / 2)
            if (pattern[pattern_x][pattern_y]) {
                cells[adjusted_x][adjusted_y] = !cells[adjusted_x][adjusted_y]
            }
        }
    }
    // Apply changes
